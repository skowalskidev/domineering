	...
	public static void main(String[] args) {
		...
		switch(args[0]){
			case "first":
				switch(args[1]){
					case "horizontal":
						board = new DomineeringBoard(true, Integer.parseInt(args[2]), Integer.parseInt(args[3]));// PLayerH starts
						board.tree().firstPlayer(new CommandLineDom());
					break;
					case "vertical":
						board = new DomineeringBoard(false, Integer.parseInt(args[2]), Integer.parseInt(args[3]));// PLayerH starts
						board.tree().firstPlayer(new CommandLineDom());
						break;
					default:
						System.exit(1);	
