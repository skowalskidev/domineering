...
public abstract class Board<Move> {
	...
	// Constructs the game tree of the board using the minimax algorithm (without alpha-beta pruning):
	...
	public GameTree<Move> minTree() {
		...
		for (Move m : availableMoves()) {
			GameTree<Move> subtree = play(m).tree();//Gets tree up to leaf with the outcome value of leaves from this tree
			children.put(m, subtree);
